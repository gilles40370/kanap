...
function appendArticleTosofa(sofa, article) {
  ...
  if (items != null) {
    items.appendChild(
      sofa
    ); /*la sa rajoute  la constante (sofa) a la section items appeller par son ID #items*/
    sofa.appendChild(
      article
    ); /* rajout d'un enfants la section items la ses l'element article*/
  }
}
...
function makeH3(name) {
  ...
  h3.textContent =
    name; /* sert as renvoyer le contenut textuelle et le cdata */
  h3.classList.add(
    "productName"
  ); /*le h3 avais une class classList et pour donner la class la ses productName */
  return h3;
}
